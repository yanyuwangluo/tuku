...
## 填 3 使用“车头模式互助模板”，本套脚本内指定前 N 个账号优先助力，N 个以后账号间随机助力。
HelpType="3"
...
export_codes_sub() {
    ...
    if cd $dir_log/$task_name &>/dev/null && [[ $(ls) ]]; then
        ...
        if [[ ${#code[*]} -gt 0 ]]; then
            ...
            3) ## 本套脚本内指定前 N 个账号优先助力，N 个以后账号间随机助力。
                HelpTemp="车头模式"
                echo -e "\n## 采用\"$HelpTemp\"互助模板"
                for ((m = 0; m < ${#pt_pin[*]}; m++)); do
                    tmp_for_other=""
...
export_all_codes() {
    ...
    if [[ $DiyHelpType = "1" ]]; then
        ...
        2)
            ...
            ;;
        3)
            echo "本套脚本内指定前 N 个账号优先助力，N 个以后账号间随机助力。"
            ;;
    	*)
            echo "按账号编号优先。"
            ;;
...
## 魔改版 jdCookie.js 复制到 /ql/scripts/。支持v2.10.8以下版本的青龙
