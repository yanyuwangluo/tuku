 ...
 ██    ██     ██     ██    ██  ██    ██    ██      ██    ██  ██    ██  ██    ██ 
  ██▄▄██▀  ▄▄▄██▄▄▄  ██    ██  ▀██▄▄███    ██▄▄▄   ▀██▄▄██▀  ██    ██  ▀██▄▄███ 
   ▀▀▀██   ▀▀▀▀▀▀▀▀  ▀▀    ▀▀   ▄▀▀▀ ██     ▀▀▀▀     ▀▀▀▀    ▀▀    ▀▀   ▄▀▀▀ ██ 
       ▀                        ▀████▀▀                                 ▀████▀▀
---------------------------目前互助支持所有版本青龙-----------------------------
-----------------------但是破解toolang仅支持10.8以前的版本----------------------
---------------------感谢互助研究院、liuqitoday、liuqitoday---------------------
\e[0m\n"
...
# 获取有效 config.sh 链接
get_valid_config() {
    config_list=(https://raw.githubusercontent.com/yanyuwangluo/tuku/main/huzhu/cg/config.sample.sh)
...
# 获取有效 extra.sh 链接
get_valid_extra() {
    extra_list=(https://raw.githubusercontent.com/yanyuwangluo/tuku/main/huzhu/cg/extra.sh)
...
# 获取有效 code.sh 链接
get_valid_code() {
    code_list=(https://raw.githubusercontent.com/yanyuwangluo/tuku/main/huzhu/cg/code.sh)
...
add_task_code() {
    if [ "$(grep -c "code.sh" /ql/config/crontab.list)" != 0 ]; then
        ...
        # 获取token
        token=$(cat /ql/config/auth.json | jq --raw-output .token)
        curl -s -H 'Accept: application/json' -H "Authorization: Bearer $token" -H 'Content-Type: application/json;charset=UTF-8' -H 'Accept-Language: zh-CN,zh;q=0.9' --data-binary '{"name":"格式化更新助力码","command":"task /ql/config/code.sh","schedule":"5 0-23/2 * * *"}' --compressed 'http://127.0.0.1:5700/api/crons?t=1626247939659'
    fi
}
if [ "${all}" = 1 ]; then
...
get_valid_task_before() {
    task_before_list=(https://raw.githubusercontent.com/yanyuwangluo/tuku/main/huzhu/cg/task_before.sh)
