...
Random_JD_COOKIE(){
    ...
    if [[ $RandomMode = "1" ]] && [[ $randomMode = "1" ]]; then
	    ...
	    if [[ $random_envs ]]; then
            ...
            for ((t = 0; t < ${#random_array[*]}; t++)); do
                local random_script="$(echo ${random_array[t]}|awk -F "@" '{print $1}')"
                local random_num="$(echo ${random_array[t]}|awk -F "@" '{print $2}')"
                [[ $local_scr =~ $random_script ]] && ran_num=$random_num
            done
        fi
...
Priority_JD_COOKIE(){
    ...
    if [[ $RandomMode && $PriorityMode ]]; then
	    ...
	    if [[ $priority_envs ]]; then
            ...
            for ((t = 0; t < ${#priority_array[*]}; t++)); do
                local priority_script="$(echo ${priority_array[t]}|awk -F "@" '{print $1}')"
                local priority_num="$(echo ${priority_array[t]}|awk -F "@" '{print $2}')"
                [[ $local_scr =~ $priority_script ]] && pri_num=$priority_num
            done
        fi
